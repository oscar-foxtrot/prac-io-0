...
def find_nwc(supply, demand, tol=1e-10):
    """
...
def find_reduced_costs(x, c, u, v, tol=1e-10):
    """
...
def find_entering(delta, x):
    ...
    с использованием правила Бленда
    input:
    x --- матрица поставок размера m x n
    delta --- матрица симплекс-множителей размера m x n
    output:
...
def pivot(x, d, tol=1e-10):
    ...
    # максимальный шаг находится из данного условия
    # где d < 0 <=> d == -1
    theta = np.min(x[d < 0])
...
import time
def solve_transportation_problem(cost, supply, demand, tol=1e-10):
    """
    ...
    tol --- параметр точности
    output:
    x_star --- матрица поставок размера m x n, задающая
...
x_opt = solve_transportation_problem(cost, supply, demand)

print("Optimal plan:")
print(x_opt)

print("Total cost:", np.sum(x_opt * cost))